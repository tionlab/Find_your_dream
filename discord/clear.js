        ...
        try {
            ...
            if (user == "NONE") {
                ...
                const embeding = new (require("discord.js")).MessageEmbed()
                    .setTitle("적으실 제목")
                    .add Field ("아래 채널로 이동해주세요!", `**${channel}**`)
                    .setTimestamp()
                    .setColor("GREEN")
                await messagechannel.bulkDelete(list).then((count) => message.channel.send({ embeds: [embeding] }).then((msg) => setTimeout(() => { msg.delete() }, 2000)))
            }
        } catch (error) {
